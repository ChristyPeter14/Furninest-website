  ...
  try {
    ...
    for (let i = 0; i < userData.cart.length; i++) {
      if (userData.cart[i].productId.quantity < 1) {
        ...
        userData.cart[i].productId.quantity < userData.cart[i].quantity
      ) {
    ...
    for (let i = 0; i < userCart.length; i++) {
      ...
      userCart[i].productId.quantity = Math.max(
        userCart[i].productId.quantity,
        0
      );
...
const cancelProduct = async (req, res) => {
  try {
    ...
    const productTotal =
      parseFloat(productInOrder.salePrice) * productInOrder.quantity;
    ...
    orderData.totalDiscountPercentage =
      (orderData.discountTotal / orderData.originalTotal) * 100;
...
const returnProduct = async (req, res) => {
  try {
    ...
    if (
      ...
      productInOrder.status === "Returned"
    ) {
      userData.wallet +=
        parseFloat(productInOrder.salePrice) * productInOrder.quantity;
      orderData.adminTotal -=
        parseFloat(productInOrder.salePrice) * productInOrder.quantity;
      await userData.save();
    }
    res.redirect("/orderDetails?id=" + orderId);
...
const showCheckOut = async (req, res) => {
  try {
    ...
    const coupon = await Coupon.find({
      is_active: true,
      "redeemedUsers.userId": { $ne: userData._id },
    });
    ...
    if (orderData.userId._id.toString() == userData._id) {
      ...
      async function processOrder(orderData) {
        for (const item of orderData.products) {
          ...
          else console.log("no product data ");
        }
      }
...
const createProductReview = async (req, res) => {
  try {
    ...
    if (!productId || !rating || !comment) {
      return res
        .status(400)
        .json({ error: "User Id,rating and commment are required" });
    }
    ...
    const message =
      productData.productReview.length > 1
        ? "Review updadated successfully"
...
const handlePaymentFailure = async (req, res) => {
  try {
    ...
    userData.cart.forEach((product) => {
      ...
      const originalPrice =
        product.productId.regularPrice * product.productId.quantity;
      const salePrice =
        product.productId.salePrice * product.productId.quantity;
      const discountAmount = originalPrice - salePrice;
      originalTotal += originalPrice;
      discountTotal += discountAmount;
    });
