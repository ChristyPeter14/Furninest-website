...
const addProducts=async (req,res)=>{
    try {
        ...
        if(!productExists){
            ...
            if(req.files &&req.files.length>0){
                for(let i=0;i<req.files.length;i++){
                    ...
                    const resizedImagePath=path.join("design","uploads","product-images",req.files[i].filename)
        ...
        console.error("Errro saving product",error)
...
const deleteSingleImage=async (req,res)=>{
    try {
        ...
        const imagePath=path.join("design","uploads","re-image",imageNameToServer)
