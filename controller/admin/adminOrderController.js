    ...
    if (newStatus === "Delivered") {
      ...
      if (productData) {
        productData.popularity =
          (productData.popularity || 0) + product.quantity; // Increment popularity by quantity
        await productData.save();
        console.log(
          `Product popularity updated. New purchase count: ${productData.popularity}`
        );
      }
    }

    if (newStatus === "Delivered") {
      const allDelivered = orderData.products.every(
        (p) => p.status === "Delivered"
      );
      if (allDelivered) {
        orderData.paymentStatus = "Success";
        console.log(
          "All products delivered. Updated payment status to Success"
        );
      }
    }
